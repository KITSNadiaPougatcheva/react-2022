...
export class GalleryUtils {
  static getMovieList(): MovieCardInterface[] {
    return movies.map(item => ({ title: item.title, img: item.img, key: item.id, range: item.range}))
  }

  static sortMovieList(movieList: MovieCardInterface[], sortBy: string): MovieCardInterface[] {
